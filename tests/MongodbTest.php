...
use God\God;
use God\Persist\Adapter\MongoDB\AdapterFiltered as Adapter;
use PHPUnit\Framework\TestCase;

/**
 * ------------------------------------------------------------------------------------
 * God MongoDB Adapter Test
 * ------------------------------------------------------------------------------------
 *
 * @author lanlin
 * @change 2018/06/25
...
class MongodbTest extends TestCase
{
    ...
    public function getDB()
    {
        ...
        $dns = "mongodb://{$option['username']}:{$option['password']}@{$option['hostname']}:";
        $dns .= "{$option['port']}/{$option['database']}";

        try
        {
            $client = new \MongoDB\Client($dns, [], $drive);

            return $client->{$option['database']}->{$option['collection']};
        }
        catch (\Exception $e)
        {
            self::fail('Unable to connect to MongoDB');
        }
    }
